...
| --- | --- | --- | --- | --- | --- |
| Email address | Email-code or password sign-in, Apple, or Google through Clerk | Yes: Clerk; Convex may receive it as a token claim. Current versions do not store it, but earlier versions could save it as a private admin label | Yes | Authentication, account display | Private. Clerk retains it while the account exists and deletes the user during in-app account deletion. Convex uses the token identifier for ownership. Public bylines and admin labels never display an email address, including legacy labels, which are deleted with the account. |
| Name | App display name chosen after first sign-in. The Apple/Google profile name is suggested; users can use any name or a generated anonymous name | Yes: Clerk and Convex | Yes | Profile display, public contributor byline, admin label | App display names are stored in Convex profiles and update existing bylines when changed. Accounts without a saved name appear as "Anonymous Skater" plus a number derived from the account identifier. Public bylines exclude email-shaped names. The profile and submitted content are deleted with the account. |
...
| Photos | Images selected from the photo library | Yes: Convex storage | Yes | Illustrate a submitted spot | Resized and re-encoded on the device before upload. Admins can add spot photos only to an empty gallery with owner permission; permission and owner notification state are private. Pending photos are private to the owner and administrators; approved photos are public. Removed when replaced, when the spot or account is deleted, or when an administrator removes the spot. Failed saves normally discard fresh uploads; any unattached remainder is removed with the uploader's account. |
...
| Reports | Reported spot, selected reason, optional details, reporter identifier, and required dead-spot evidence photos | Yes: Convex and its storage | Yes | Private review, safety, and abuse handling | Reports and evidence are private to admins; photo upload access is checked against the reporter. Reports and evidence are deleted after resolution, spot deletion, or reporter account deletion. Unattached evidence uploads expire after 24 hours. Resolved removals retain a count, not reporter identities or evidence. |
