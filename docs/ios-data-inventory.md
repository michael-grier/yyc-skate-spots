...
| Name | Optional app display name, or Apple/Google profile name from Clerk | Yes: Clerk and Convex | Yes | Profile display, public contributor byline, private admin label | App display names are stored in Convex profiles and update existing bylines when changed. Provider names are used when no app name exists. Public bylines exclude email-shaped names. The profile and submitted content are deleted with the account. |
