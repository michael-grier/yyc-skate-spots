...
| --- | --- | --- | --- | --- | --- |
| Email address | Email-code or password sign-in, Apple, or Google through Clerk | Yes: Clerk; Convex may receive it as a token claim but does not store it | Yes | Authentication, account display | Private. Clerk retains it while the account exists and deletes the user during in-app account deletion. Convex uses the token identifier for ownership. Public bylines and admin labels never use the email address. |
